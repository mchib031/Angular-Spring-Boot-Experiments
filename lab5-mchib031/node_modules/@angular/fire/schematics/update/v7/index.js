...
const ngUpdate = () => (host, context) => {
    const packageJson = host.exists('package.json') && ng_add_common_1.safeReadJSON('package.json', host);
    if (packageJson === undefined) {
        throw new schematics_1.SchematicsException('Could not locate package.json');
    }
    Object.keys(versions_json_1.default).forEach(depName => {
        const dep = versions_json_1.default[depName];
        if (dep.dev) {
    ...
    Object.keys(versions_json_1.firebaseFunctions).forEach(depName => {
        const dep = versions_json_1.firebaseFunctions[depName];
    ...
    ng_add_common_1.overwriteIfExists(host, 'package.json', ng_add_common_1.stringifyFormatted(packageJson));
    context.addTask(new tasks_1.NodePackageInstallTask());
    const angularJson = host.exists('angular.json') && ng_add_common_1.safeReadJSON('angular.json', host);
    if (packageJson === undefined) {
    ...
    host.visit(filePath => {
        ...
        if (content !== newContent) {
            ng_add_common_1.overwriteIfExists(host, filePath, newContent);
        }
    });
    return host;
};
